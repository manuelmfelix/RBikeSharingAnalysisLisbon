...
# setwd("//mfelix/TISpt/RScripts/R_Apps/App2")
...
analiseGiras <- DBI::dbReadTable(con, 'giras_Nova')
...
ggplot(sumatorio, mapping=aes(x=time, y=numbicicletasSum, group = add_data, color=as.factor(add_data))) +
  geom_line() +
  labs(title="", x="", y="") +
...
write.csv(sumatorio,file = "sumatorio.csv")
